...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) {
    ...
    Eigen::Vector4f _v[3]; //存储三角形三个点坐标 x, y, z

    int ind = 0;
    int ll = 1e8, rr = -1e8, tt = -1e8, bb = 1e8;
    for (auto& vec: v) {
        ll = std::min(ll, int(vec.x()));
        rr = std::max(rr, int(vec.x()));
        bb = std::min(bb, int(vec.y()));
        tt = std::max(tt, int(vec.y()));
        // std::cout << vec.x() << ' ' << vec.y() << std::endl;
        Eigen::Vector4f vv(vec.x(), vec.y(), vec.z(), 1.0f);
        _v[ind] = vv;
        ind++;
    }
    ...
    for (int i = ll; i <= rr; i++) {
        for (int j = bb; j <= tt; j++) {
            ...
            if ((!SMAA) && insideTriangle(i, j, _v)) { //如果i,j在三角形内
                ...
                auto [alpha, beta, gamma] = computeBarycentric2D(i, j, t.v); //计算重心坐标的alpha, beta, gamma
                float w_reciprocal = 1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
                float z_interpolated = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w(); //计算插值后的z值
            ...
            // else if (SMAA) {
            //     int cnt = 0;
            //     if (insideTriangle(i + .33, j + .33, _v))
            //         ++cnt;
            //     if (insideTriangle(i + .25, j + .67, _v))
            //         ++cnt;
            //     if (insideTriangle(i + .67, j + .33, _v))
            //         ++cnt;
            //     if (insideTriangle(i + .67, j + .67, _v))
            //         ++cnt;
            //     if (cnt) {
            ...
            //             // if(cnt != 4)
            //             //     std::cout << cnt << std::endl;
            //             set_pixel(point, t.getColor() * cnt / 4.);
            //             // if(cnt != 4){
            //             //     Eigen::Vector3f white(254, 254, 254);
            //             //     set_pixel(point, white);
            //             //     }
            //         }
            //     }
            // }
